...
localStorage.setItem('totalQuesPresent-11',43);
...
const save = () => {
  ...
  for (let i = 0; i < checkBoxes.length; i++) {
    if (checkBoxes[i].checked) {
      localStorage.setItem("key-graph" + i, 1);
      count = count + 1;
    } else {
      localStorage.setItem("key-graph" + i, 0);
    }
  }
  localStorage.setItem('totalQuesDone-11',count);
};

//Function to load the current state of the check boxes
const load = () => {
  for (let i = 0; i < checkBoxes.length; i++) {
    let temp = localStorage.getItem("key-graph" + i);
    // console.log(temp);
    if (temp == 1) {
