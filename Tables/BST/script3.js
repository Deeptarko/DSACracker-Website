...
localStorage.setItem('totalQuesPresent-21',43);
...
const save = () => {
  ...
  for (let i = 0; i < checkBoxes.length; i++) {
    if (checkBoxes[i].checked) {
      localStorage.setItem("key-bst" + i, 1);
      count = count + 1;
    } else {
      localStorage.setItem("key-bst" + i, 0);
    }
  }
  localStorage.setItem('totalQuesDone-6',count);
};

//Function to load the current state of the check boxes
const load = () => {
  for (let i = 0; i < checkBoxes.length; i++) {
    let temp = localStorage.getItem("key-bst" + i);
    // console.log(temp);
    if (temp == 1) {
