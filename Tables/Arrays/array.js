...
const save = () => {
  ...
  for (let i = 0; i < checkBoxes.length; i++) {
    if (checkBoxes[i].checked) {
      localStorage.setItem("key-array" + i, 1);
      count = count + 1;
    } else {
      localStorage.setItem("key-array" + i, 0);
    }
  }
  localStorage.setItem('totalQuesDone-0',count);
};

//Function to load the current state of the check boxes
const load = () => {
  for (let i = 0; i < checkBoxes.length; i++) {
    let temp = localStorage.getItem("key-array" + i);
    // console.log(temp);
    if (temp == 1) {
